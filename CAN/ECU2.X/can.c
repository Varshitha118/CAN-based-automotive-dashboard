...
void init_can(void)
{
   ...
   * Filter 0 = 0xFFC
   */
  //filter setting for 0x70
  RXF0SIDH = 0x70 >> 3; //0X70
  RXF0SIDL = 0x70 << 5;
...
void can_transmit(unsigned int msg_id, unsigned int data)
{
  ...
  TXB0DLC = 0x02;			/* Data Length Count */
  TXB0D0 = (data >> 8) & 0xFF;			/* DataByte 0 */
  TXB0D1 = data & 0xFF;			/* DataByte 1 */
  TXB0D2 = 0;			/* DataByte 2 */
  TXB0D3 = 0;			/* DataByte 3 */
  TXB0D4 = 0;			/* DataByte 4 */
  TXB0D5 = 0;			/* DataByte 5 */
